...
4. Update `campaigns/<campaign>/_index.md` — add wikilinks under the appropriate sections for any newly created entities.
...
3. **If no matching session exists:** create `Session NN — <RP Title>.md` where `NN` is the next unused session number. Use frontmatter:
   ...
   ---
   ```
   Body: short summary, key beats, links to NPCs/locations/items touched.
